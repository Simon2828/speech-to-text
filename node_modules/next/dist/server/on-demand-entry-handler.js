...
const utils_1 = require("../build/webpack/utils");
...
function onDemandEntryHandler(devMiddleware, multiCompiler, { buildId, dir, dev, reload, pageExtensions, maxInactiveAge = 1000 * 60, pagesBufferLength = 2, wsPort }) {
        ...
        compiler.hooks.make.tapPromise('NextJsOnDemandEntries', (compilation) => {
            ...
            const allEntries = Object.keys(entries).map(async (page) => {
                ...
                for (const file of files) {
                    try {
                        await access(path_1.join(dir, file), (fs_1.default.constants || fs_1.default).W_OK);
                    }
                    catch (err) {
    ...
    return {
        ...
        async ensurePage(page) {
            ...
            try {
                normalizedPagePath = require_1.normalizePagePath(page);
            }
            catch (err) {
                console.error(err);
            ...
            const pathname = path_1.join(dir, relativePathToPage);
            const { name, files } = utils_1.createEntry(relativePathToPage, { buildId, pageExtensions: extensions });
